...
describe('reducerToggleBooleanField', () => {
  it('Should throw on field not existing', () => {
    ...
    expect(() => reducerToggleBooleanField({}, 'field_not_exist')).toThrow(
      `Field '${strFieldName}' does not exist on current state, and hence cannot be toggled.`,
    );
  });
  ...
  it('Should throw on field not being a boolean', () => {
    ...
    expect(() =>
      reducerToggleBooleanField(objReducerState, strFieldName),
    ).toThrow(
      `Invalid type for toggling field '${strFieldName}'. Expected 'boolean', got 'string'`,
    );
  });
  ...
  it('Should toggle a boolean field', () => {
    ...
    expect(reducerToggleBooleanField(objInitialState, strFieldName)).toEqual(
      objExpectedState,
    );
  });
});
